...
// Anthropic API 키가 없으면 few-shot 샘플 데이터를 반환합니다.
import exerciseDb from '../data/exerciseDb.json';
import StorageService from './storageService';

// ─── 부위 순서 (build_prompt_snippet 포팅) ───────────────────────
...
// ─── Few-shot 예시 (wefit_ai_pipeline.py에서 그대로 포팅) ────────
const FEWSHOT_STRENGTH_INPUT = {
  name: '김민준', age: 24, gender: '남', height: 178, weight: 78,
  muscleMass: 30, bmi: 24.63, goal: '근력_상승', availableDays: 5,
};
...
const FEWSHOT_DIET_INPUT = {
  ...
  muscleMass: 15, bmi: 22.58, goal: '다이어트', availableDays: 5,
};
...
function formatUserMessage(u) {
  const goalLabel = u.goal === '근력_상승' ? '근력 상승' : '다이어트';
  return (
    `이름: ${u.name} / 나이: ${u.age}세 / 성별: ${u.gender}\n` +
    `키: ${u.height}cm / 몸무게: ${u.weight}kg / 근력량: ${u.muscleMass ?? '미입력'}kg / BMI: ${u.bmi}\n` +
...
const AIService = {
  async generateWorkout(userData) {
    // .env의 VITE_GEMINI_API_KEY 우선, 없으면 앱 설정의 localStorage 값 사용
    const envKey  = import.meta.env.VITE_GEMINI_API_KEY ?? '';
    const rawKey  = envKey.trim() || StorageService.get('gemini_api_key');
    const apiKey  = typeof rawKey === 'string' ? rawKey.trim() : '';

    // API 키 없으면 few-shot 샘플 반환
    if (!apiKey) {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        contents: [
          ...
          { role: 'user',  parts: [{ text: formatUserMessage(userData) }] },
        ],
        generationConfig: {
          temperature: 0.7,
