// 서버 전용 — 운동 처방 프롬프트 빌더 (구 aiService.js 로직 이전)
// 클라이언트 번들에 포함되지 않음 (api/ 는 서버리스 함수로만 실행)
import exerciseDb from '../../src/data/exerciseDb.json' with { type: 'json' };
...
- reps는 반드시 단일 정수 (예: 8). "6~8" 같은 범위 문자열 금지
- sets도 단일 정수
- rest는 반드시 "초" 단위 문자열 (예: "90초"). "분" 사용 금지 (180초로 환산)
- duration은 플랭크/유산소에만 포함하며 "초" 또는 "분" 문자열 (예: "60초", "30분")`.trim();
}

export function formatUserMessage(u, completedHistory = null) {
...
// ─── API 실패 시 폴백 예시 처방 ───────────────────────────────────
export const FALLBACK_STRENGTH = {
  weeklyRoutine: {
